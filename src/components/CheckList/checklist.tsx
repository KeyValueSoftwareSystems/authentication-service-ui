...
export const ChecklistComponent: FC<ChecklistProps> = ({
    ...
    const removeGroup = (item: string) => {
        ...
        setState([
            ...atomKey.slice(0, itemIndex),
            ...atomKey.slice(itemIndex + 1)
        ]);
    }
    ...
    return (
        ...
        <div id="add-groups">
            <legend id="bold"> {name} </legend>
            {
                mapList.map((item: any) => {
                    return (
                        <div id="checkbox"> <label><input type="checkbox" key={item.id} defaultChecked={isChecked(item.id)} onChange={(event: any) => handleChange(event, item)} />{item.name}</label><br></br></div>
                    )
                })
            }
        </div>
