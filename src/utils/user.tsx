...
export const AddUserformSchema = yup.object({
  ...
  email: yup.string().email('Invalid email').required('Email cannot be empty').max(40),
  phone: yup
    .string()
  ...
  return (
    <>
      ...
      <UserDetails desktopScreen={isDesktopScreen}>
        ...
        <div className={row.status !== 'INVITED' ? 'email' : 'blurred-email'}>{row.email}</div>
      </UserDetails>
    </>
  );
};
  ...
  },
  {
    ...
    renderCell: (params) => (
      <div className='group-list'>
        <TableChipElement rowItems={params} columnName='groups' defaultSize={3} />
      </div>
    ),
    headerAlign: 'left',
    sortable: false
